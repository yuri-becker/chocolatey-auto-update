...
{
    $mbresult = [System.Windows.Forms.MessageBox]::Show("This script needs an additional module called 'BurntToast'. Install it?", 'Chocolatey Auto-Update', 'OkCancel', 'Information')
    if($mbresult -eq 'OK') {
...
if ($matches.Count -gt 0) {
    $mbresult = [System.Windows.Forms.MessageBox]::Show("$matches`nDo you want to update?", 'Chocolatey', 'YesNo','Question')
    if($mbresult -eq 'Yes') {
